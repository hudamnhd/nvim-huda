  ...
  for _, line in ipairs(output) do
    ...
    if d1 then
      ...
      if dc > 0 and ac > 0 then
        ...
        if dc > ac then removed = removed + (dc - ac)
        elseif ac > dc then added = added + (ac - dc) end
      elseif dc > 0 then removed = removed + dc
      elseif ac > 0 then added = added + ac end
    end
  end
...
local function get_diagnostic()
  ...
  for i = 1, 4 do
    ...
    if count > 0 then
      table.insert(parts, string.format('%s%d', icons[i], count))
    end
  end
