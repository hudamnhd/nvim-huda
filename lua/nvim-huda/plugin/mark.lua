-- taken from https://github.com/lewis6991/dotfiles/blob/0071d6f1a97f8f6080eb592c4838d92f77901e84/config/nvim/lua/gizmos/marksigns.lua

local ns_id = vim.api.nvim_create_namespace('nvimhuda/mark')

---@param bufnr integer
---@param mark vim.fn.getmarklist.ret.item
local function decor_mark(bufnr, mark)
  pcall(vim.api.nvim_buf_set_extmark, bufnr, ns_id, mark.pos[2] - 1, 0, {
...
vim.api.nvim_set_decoration_provider(ns_id, {
  on_win = function(_, _, bufnr, top_row, bot_row)
    ...
    vim.api.nvim_buf_clear_namespace(bufnr, ns_id, top_row, bot_row)
